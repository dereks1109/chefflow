...
- 2026-05-24: Path-B same-day stack — four launch-prep wins. (1) `HelpSheet` + UserButton menu entry on desktop/mobile + mailto-draft to `support@chefflow.com`. (2) `.github/workflows/ci.yml` running lint (non-blocking) + tests + build on PRs, with concurrency cancel. (3) `DEPLOY.md` runbook + `chefflow/.env.example` + `chefflow-worker/.dev.vars.example` + unauthenticated `/api/health`. (4) Sentry on SPA + Worker with strict PII scrubbers (drops `ui.input` / `console` breadcrumbs, strips `request.data` on `/api/{sync,account,llm}/*`), `AppErrorBoundary` wrapping the React tree, `x-request-id` correlation header surfaced via Cloudflare's `withSentry()` wrap; non-network sync failures auto-report tagged `kind=sync`. Code is env-gated — deploying without a DSN is a no-op.
